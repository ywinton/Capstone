...
predictword3 <- function(x, y, z, n = 10) {
  ...
  if(x == "" & y == "" & z == "") {
    ...
    # First use trigram library, match second input word with word1 
    # and third input word with word2 in trigram library
    
  }   else if(y %in% df3$word1 & z %in% df3$word2) {
    predict <-  filter(df3,word1 %in% y & word2 %in% z) %>%
      select(Next, freq)
    
    # Then use quadgram library, match first input word with word1 
    # and second input word with word2 and third input word with word3 in quadgram library
    
  }   else if(x %in% df4$word1 & y %in% df4$word2  & z %in% df4$word3) {
    predict <-  filter(df4,word1 %in% x & word2 %in% y & word3 %in% z) %>%
      select(Next, freq)
     
    # If not found in trigram or quad library, switch to bigram library, match   
    # third input word with word1 in bigram library
  }   else if(z %in% df2$word1) {
    predict <- filter(df2, word1 %in% z) %>%
      select(Next, freq)
    
    # If not found in quadgram, trigram and bigram libraries, return top unigram words
  }   else{
    predict <- select(df1, Next, freq)
